...
import via_lactea from "../recursos/2k_stars_milky_way.jpg"

import guiSistemaSolar from "./guiTextureSistemaSolar.json"
import guiSol from "./guiSol.json"
...
const onSceneReady = (e) => {
    ...
    let guiTextureSistemaSolar = advancedTexture.parseSerializedObject(guiSistemaSolar)
    ...
    var meshes = [sol, mercurio, venus, tierra, marte, jupiter, saturno, urano, neptuno, luna, saturnoAnillos]

    const XR = XR_Module.XR_Experience(ground, skybox, scene);

    // AL hacer click en el sol mostrar una ventana con su informacion
    sol.actionManager = new Babylon.ActionManager(scene)
    sol.actionManager.registerAction(new Babylon.ExecuteCodeAction(Babylon.ActionManager.OnPickTrigger, () => {

        let advancedTexture2 = GUI.AdvancedDynamicTexture.CreateFullscreenUI("UI2", true, scene)
        advancedTexture2.idealWidth = 1920
        advancedTexture2.idealHeight = 1080
        
        let guiTextureSol = advancedTexture2.parseSerializedObject(guiSol)

        let vSol = advancedTexture2.getControlByName("vSol")
        let bCerrar = advancedTexture2.getControlByName("BCerrar")
        let TBCerrar = advancedTexture2.getControlByName("TBCerrar")

        bCerrar.onPointerClickObservable.add(() => {

            vSol.isVisible = false
